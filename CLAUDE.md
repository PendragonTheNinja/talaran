...
- **After a push, verify the delivery file by file** against `origin/main`, not by the commit message. `059c87d`'s message listed the rate-limiter fix, but `middleware/rateLimit.ts` was not in it, and only a later proof run noticed (2026-09-25).
...
11. **The database refuses negatives (audit §5.1).** CHECK constraints (migration `20260925180000_nonnegative_checks`) hold `players.gold`, `player_inventory`, `property_storage`, `ground_items`, `shop_listings`, `trade_offers` and `trade_gold` at `>= 0`, `player_shops` till and buy fund at `>= 0`, and `shop_buy_orders.quantity_filled` between 0 and `quantity_wanted`. A write that would break one throws Postgres error `23514` and the statement fails. That is the backstop, not the check: validate and lock first so players get a real refusal, and treat a `23514` in the logs as a race to fix. The rows are `>= 0`, not `> 0`, until every path deletes a stack that reaches zero.
12. **Money records outlive accounts; Paddle drives Talers both ways (audit M5).** `taler_purchases`, `taler_ledger` and `taler_adjustments` are `ON DELETE RESTRICT` from `players`: never delete a player who holds them (the guest sweep skips such guests and logs them). The webhook credits every item at its tier times its quantity, refuses guests and unknown players (logged for a manual refund), and applies approved refunds, credits and chargebacks through `applyPaddleAdjustment` in `services/talers.ts`, which takes back the share they cover, once per adjustment id, never more than the purchase gave; reversals restore it. A balance a chargeback drives negative blocks spending until covered.
...
### Other tick facts
- **Absence cancels after 30 minutes.** `lib/presence.ts` deletes an action at resolution time once the player has had no socket for `OFFLINE_GRACE_MS`, measured from `players.last_seen`. That column is persisted, so a deploy never resets anyone's allowance: it is stamped on join, on every disconnect, and once a minute for everyone connected by the playtime flush in `services/playtime.ts` (the heartbeat that makes a crash safe). `RESTART_GRACE_MS` (90s) forgives everyone just after boot. The same column is the guild list's "Last seen". **Presence reads socket rooms (audit M10).** `isOnline`/`onlinePlayers()` in `lib/realtime.ts` and `isPlayerOnline` in `lib/presence.ts` all count `player_<id>` room membership, so several tabs are handled and closing one never marks a player offline; `isPlayerOnline` also consults an explicit set as a second signal. **Chat rooms come from the server (audit M6):** `join_location` ignores anything the client sends and joins `location_<current_location_id>` plus its region.
...
- **One action per player:** `player_actions.player_id` is unique on live (`player_actions_player_id_unique`). Actions start with a `busy()` check and then a plain insert (17 call sites), so that index is the real gate: a second start that races past the check fails on it. Tests needing many due actions need many players.
...
- **The origin only answers Cloudflare (audit H8, second half).** The Hetzner Cloud Firewall `talaran-alpha` admits 80 and 443 from Cloudflare's published ranges only (IPv4 and IPv6), and 22 from anywhere; nothing else, so Node's 3000 is closed. `clientKey` in `middleware/rateLimit.ts` trusts `CF-Connecting-IP` BECAUSE of this rule: if the origin ever answers anyone else, that header is attacker-chosen and the auth and guest limits can be sidestepped. Never open 80/443 to `0.0.0.0/0`. If Cloudflare changes its ranges (https://www.cloudflare.com/ips), refresh the rule through the Hetzner API (`POST /v1/firewalls/{id}/actions/set_rules`), not by hand. Check from outside: `curl --max-time 5 --resolve talaran.net:443:<origin IP> https://talaran.net/health` must time out. Keep the origin IP out of the repo and public channels.
