...
const HUNT_YOUNG: Record<string, { itemName: string; min: number; max: number; chance: number; notable: boolean }[]> = {
    // Calves turn up beside anything — a strayed drove animal is not fussy about
    // which wood it wanders into. Foals overlap by tier instead: the rouncey is
    // the common working horse and shows up in the shallower hunts, the palfrey
    // is finer and keeps to the deeper ones, and Boar sits in the middle where
    // both are possible.
    Deer: [
...
export async function up(knex: Knex): Promise<void> {
    ...
    for (const [animalName, additions] of Object.entries(HUNT_YOUNG)) {
        ...
        // Drop any previous live-young entries, then re-add: makes the migration
        // idempotent AND re-runnable after a rebalance, rather than skipping rows
        // that already exist at the old numbers.
        const young = new Set(['Calf', 'Rouncey Foal', 'Palfrey Foal']);
        const table: any[] = JSON.parse(animal.drop_table || '[]').filter((d: any) => !young.has(d.itemName));
        for (const add of additions) table.push(add);
        await knex('huntable_animals').where({ id: animal.id }).update({ drop_table: JSON.stringify(table) });
    }

    // ── The Stockman's Seminar — Geothro of Novita ──
    ...
    let quest = await knex('quests').where({ name: "The Stockman's Seminar" }).first();
    if (!quest) {
        [quest] = await knex('quests').insert({
            name: "The Stockman's Seminar",
    ...
    // Start items cover the whole first loop: the pail to feed with, the fork and
    // straw for the bedding, two chicks, and grain enough to get them laying.
    // Two chicks eat 2 grain a feed and want 2 straw a mucking, so this is about
    // ten feeds and five muckings — enough to learn the rhythm, not enough to
    // coast on.
    ...
    await knex('npc_dialogues').insert([
        ...
        },
        {
            ...
            options: JSON.stringify([
                { label: 'Accept Quest', next_stage: 'progress', action: "start_quest:The Stockman's Seminar" },
        ...
        },
        {
            ...
            options: JSON.stringify([
                { label: '"Thank you, Geothro."', next_stage: 'complete', action: "complete_talk_objective:The Stockman's Seminar" },
            ]),
        },
        {
            npc_id: geothro.id,
...
export async function down(knex: Knex): Promise<void> {
    ...
    const quest = await knex('quests').where({ name: "The Stockman's Seminar" }).first();
    if (quest) {
